...
namespace PashOS.Interpreter
{
    ...
    public class MathParser
    {
            ...
            bool loadPreDefinedVariables = true)
        {
            ...
            this.LocalVariables = new Lib.Dictionary<string, int>();
            this.LocalFunctions = new Lib.Dictionary<string, Func<int[], int>>();
            this.OperatorAction = new Lib.Dictionary<string, Func<int, int, int>>();
            this.OperatorList = new System.Collections.Generic.List<string>();
            if (loadPreDefinedOperators)
            {
                ...
                this.OperatorAction.Add("^", (numberA, numberB) => (int)Math.Pow((double)numberA, (double)numberB));
                this.OperatorAction.Add(":", (numberA, numberB) => numberA / numberB);
                this.OperatorAction.Add("/", (numberA, numberB) => numberA / numberB);
                this.OperatorAction.Add("*", (numberA, numberB) => numberA * numberB);
                this.OperatorAction.Add("+", (numberA, numberB) => numberA + numberB);
            ...
            if (loadPreDefinedFunctions)
            {
                ...
                this.LocalFunctions.Add("cosh", x => (int)Math.Cosh((double)x[0]));
                this.LocalFunctions.Add("arccos", x => (int)Math.Acos((double)x[0]));

                this.LocalFunctions.Add("sin", x => (int)Math.Sin((double)x[0]));
                this.LocalFunctions.Add("sinh", x => (int)Math.Sinh((double)x[0]));
                this.LocalFunctions.Add("arcsin", x => (int)Math.Asin((double)x[0]));

                this.LocalFunctions.Add("tan", x => (int)Math.Tan((double)x[0]));
                this.LocalFunctions.Add("tanh", x => (int)Math.Tanh((double)x[0]));
                this.LocalFunctions.Add("arctan", x => (int)Math.Atan((double)x[0]));

                this.LocalFunctions.Add("sqrt", x => (int)Math.Sqrt((double)x[0]));
                this.LocalFunctions.Add("rem", x => (int)Math.IEEERemainder((double)x[0], (double)x[1]));
                this.LocalFunctions.Add("round", x => (int)Math.Round((double)x[0]));
                this.LocalFunctions.Add("pow", x => (int)Math.Pow((double)x[0], (double)x[1]));

                this.LocalFunctions.Add("rndm", x => (int)new Random().Next((int)x[0], (int)x[1]));
                this.LocalFunctions.Add("factorial", x => (int)this.Factorial((int)x[0]));
            }

            if (loadPreDefinedVariables)
            {
                // local variables such as pi can also be added into the parser.
                this.LocalVariables.Add("pi", (int)Math.PI);
                this.LocalVariables.Add("e", (int)Math.E);
            }
        }
        ...
        /// </summary>
        public Lib.Dictionary<string, Func<int[], int>> LocalFunctions { get; set; }

        /// <summary>
        ///     All variables that you want to define should be inside this property.
        /// </summary>
        public Lib.Dictionary<string, int> LocalVariables { get; set; }

        /// <summary>
        ///     When adding a variable in the OperatorList property, you need to assign how that operator should work.
        /// </summary>
        public Lib.Dictionary<string, Func<int, int, int>> OperatorAction { get; set; }
        ...
        /// </summary>
        public System.Collections.Generic.List<string> OperatorList { get; set; }
        ...
        /// <param name="mathExpression"></param>
        /// <returns></returns>
        public int ProgrammaticallyParse(string mathExpression)
        {
            if (mathExpression.Contains("="))
            {
                ...
                int varValue = this.Parse(mathExpression);
        ...
        private int Scanner(string _expr)
        {
            // SCANNING THE INPUT STRING AND CONVERT IT INTO TOKENS

            var _tokens = new System.Collections.Generic.List<string>();
            string _vector = "";
           ...
           /* _expr = this.LocalVariables.Aggregate(
            ...
            {
                LocalVariables.Keys[i] = "(" + LocalVariables.Values[i].ToString(this.CULTURE_INFO) + ")";
            }
